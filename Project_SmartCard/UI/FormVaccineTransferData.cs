...
{
    public partial class FormVaccineTransferData : DevExpress.XtraEditors.XtraForm
    {
        ...
        public FormVaccineTransferData()
        {
            InitializeComponent();
        }

        private void FormVaccineTransferData_Load(object sender, EventArgs e)
        {
        }
        ...
        private void newPT_FromLocalhost()
        {
            ...
            dtNewPT = LocahostlDataAccess.RetrieveData(sqlNewPT);
            ...
            if (dtNewPT != null && dtNewPT.Rows.Count > 0)
            {
                for (int i = 0; i < dgvNewPT.Rows.Count; i++)
                {
                    ...
                    if (dtCheckPT.Rows.Count > 0)
                    {
                        ...
                        LocahostlDataAccess.ExecuteSQL(update_Local_OPD_OPD_dummy);
                        ...
                        LocahostlDataAccess.ExecuteSQL(update_Local_OPD_OPD);
                    }
                    // insert into pt.pt for new PT
                    else
                    {
                        string hn = LocahostlDataAccess.MaxHN();
                        ...
                        try
                        {
                            ...
                            LocahostlDataAccess.ExecuteSQL(sql_Update_New_HN_dummy);
                            ...
                            LocahostlDataAccess.ExecuteSQL(sql_Update_New_HN);
                            ...
                            LocahostlDataAccess.ExecuteSQL(sql_Update_HOsdata_Docno_MaxHN1);
                        }
                        catch (Exception ex)
                        {
            ...
            dtOPD_LOCAL = LocahostlDataAccess.RetrieveData(sql_SELECT_FROM_OPD_LOCAL);
            ...
            this.Invoke(new ThreadStart(delegate
            {
                foreach (DataRow row in dtOPD_LOCAL.Rows)
                {
                    ...
                    if (dtHN.Rows.Count > 0)
                    {
                        ...
                        LocahostlDataAccess.ExecuteSQL(sql_UPDATE_HN);
                    }
            ...
            this.Invoke(new ThreadStart(delegate { dgvOPD_OPD.DataSource = LocahostlDataAccess.RetrieveData(sql_OPD_AFTER_UPDATE_HN); }));
        ...
        private void bgwOPD_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            this.BeginInvoke(new ThreadStart(delegate { LocahostlDataAccess.UpdateHN_Hosdata_Docno_Local(); }));
